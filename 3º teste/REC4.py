...
#Euler-crommer
for i in range(n):
    ax[i]=-(k/m)*x[i]-3*(alpha/m)*x[i]**2 -(b/m)*x[i]**3
    vx[i + 1] = vx[i] + ax[i] * dt
    x[i + 1] = x[i] + vx[i+1] * dt
    
    Ep[i]= 0.5*k*x[i]**2 + alpha*x[i]**3 - b*x[i]**4
    # Ep[i]= 0.5*k*X[i]**2 + b*X[i]**4        #Para calcular para um x arbitrario
    Ec[i]= 0.5*m*vx[i]**2
...
print("WAVE RELATED:")
print("Movimento:")
print("x in ]{:0.3f},{:0.3f}[ m".format(xmin,xmax))
print("Amplitude:")
print("A = ]{:0.3f},{:0.3f}[ m".format(Amp[0],Amp[-1]))
print("Periodo:")
print("T = ]{:0.3f},{:0.3f}[ s".format(periodos[0],periodos[-1]))
print("Frequencia:")
print("f = ]{:0.3f},{:0.3f}[ Hz".format(freq[0],freq[-1]))
print("=================================")
print("ENERGY:")
print("Mechanical:")
...
#================================================
#ENERGIA POTENCIAL
plt.plot(X, Ep, "-m", label = "Ep")
plt.grid()
plt.title("Variacao da Eneria Potencial")

plt.plot(X, X*0+4, "-b", label = "Ep = 4")
